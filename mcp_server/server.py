...
@mcp.tool(name="get_user_by_id", description="Retrieves user details by identifier.")
async def get_user_by_id(user_id: int) -> str:
    return await user_client.get_user(user_id=user_id)


@mcp.tool(name="delete_user", description="Deletes a user for the provided identifier.")
...
@mcp.tool(
    name="search_user",
    description="Searches users by first name, last name, email, or gender.",
)
...
@mcp.tool(name="add_user", description="Creates a new user with the supplied payload.")
async def add_user(user: UserCreate) -> str:
    return await user_client.add_user(user_create_model=user)


@mcp.tool(name="update_user", description="Updates an existing user using the provided payload.")
async def update_user(user_id: int, payload: UserUpdate) -> str:
    return await user_client.update_user(user_id=user_id, user_update_model=payload)


@mcp.resource(
    uri="users-management://flow-diagram",
    description="Static diagram of the user-service request flow.",
    mime_type="image/png",
)
async def get_flow_diagram() -> bytes:
    flow_path = Path(__file__).with_name("flow.png")
    if not flow_path.exists():
        raise FileNotFoundError("flow.png resource is missing")
    return flow_path.read_bytes()


SEARCH_ASSIST_PROMPT = """
...
@mcp.prompt(
    name="search-strategy-helper",
    description="Helps formulate effective user search queries.",
)
async def search_strategy_helper() -> str:
    return SEARCH_ASSIST_PROMPT


@mcp.prompt(
    name="profile-creation-guide",
    description="Guides the creation of realistic user profiles.",
)
